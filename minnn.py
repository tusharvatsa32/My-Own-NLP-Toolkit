...
class Tensor:
    ...
    def accumulate_grad_sparse(self, gs: List[Tuple[int, xp.ndarray]]) -> None:
        if self.grad is not None:
            rand_gs=[]
            for element in range(len(gs)):
                rand_gs.append(gs[element])
            self.dict=self.grad
            for element in range(len(gs)):
                if rand_gs[element][0] not in self.dict:
                    self.dict[rand_gs[element][0]]=rand_gs[element][1]
                else:
                    self.dict[rand_gs[element][0]]=self.dict[rand_gs[element][0]]+[element][1]
                    print('hi')
            gs=rand_gs
            self.grad=self.dict
        else:
            ...
            for element in range(len(gs)):
                if rand_gs[element][0] not in self.dict:
                    ...
                    self.dict[rand_gs[element][0]]=self.dict[rand_gs[element][0]]+rand_gs[element][1]
            gs=rand_gs
            self.grad=self.dict
...
class Initializer:
    ...
    def xavier_uniform(shape: Sequence[int], gain=1.0):
        return xp.random.uniform(low=-1,high=1,size=shape)*math.sqrt(6./(shape[0]+shape[1]))
...
class MomentumTrainer(Trainer):
    ...
    def update(self):
        ...
        mrate=self.mrate
        self.iter=0
        self.m=[None for _ in range(len(self.model.params))]
        for p in self.model.params:
            if p.grad is not None:
                if isinstance(p.grad, dict):  # sparsely update to save time!
                    if self.m[self.iter] is None:
                        self.m[self.iter]=xp.zeros_like(p.data)
    ...
    def update_dense(self, p: Parameter, g: xp.ndarray, lrate: float, mrate:float):
        ...
        self.m[self.iter]=mrate*self.m[self.iter]+(1-mrate)*g
        p.data-=self.m[self.iter]
        # else:
        #     self.m=(1-mrate)*g
        #     print(self.m.shape,'self.m.shape Initial')
        #     p.data-=self.m

        
    def update_sparse(self, p: Parameter, gs: Dict[int, xp.ndarray], lrate: float,mrate:float):
        for widx, arr in gs.items():
            p.data[widx] -= lrate * arr+mrate*self.m[widx]
...
class OpLookup(Op):
    def __init__(self):
        super().__init__()
    def forward(self,W_emb:Tensor,idx:List):
        one_hot_matrix=xp.zeros((len(idx),W_emb.data.shape[0]))
        for element in range(len(idx)):
            one_hot_matrix[element][idx[element]]=1
        output_t=Tensor(xp.matmul(one_hot_matrix,W_emb.data))
        self.store_ctx(emb=W_emb,idx=idx,one_hot_matrix=one_hot_matrix,output_t=output_t)
        return output_t
    
    def backward(self):
        emb, idx,one_hot_matrix,output_t=self.get_ctx('emb','idx','one_hot_matrix','output_t')
        if output_t.grad is not None:
            grad_t=xp.matmul(xp.transpose(one_hot_matrix),output_t.grad)
            emb.accumulate_grad(grad_t)
...
class OpDot(Op):
    def __init__(self):
        super().__init__()
    def forward(self,W:Tensor,t:Tensor):
        output=xp.matmul(W.data,t.data)
        output=Tensor(output)
        self.store_ctx(W=W,t=t,output=output)
        return output
    def backward(self):
...
class OpTanh(Op):
    ...
    def forward(self,t:Tensor):
        arr_tanh=(xp.exp(t.data)-xp.exp(-t.data))/(xp.exp(t.data)+xp.exp(-t.data))
        t_tanh=Tensor(arr_tanh)
        self.store_ctx(t=t,t_tanh=t_tanh,arr_tanh=arr_tanh)
        return t_tanh
    def backward(self):
        t,t_tanh,arr_relu=self.get_ctx('t','t_tanh','arr_tanh')
        if t_tanh.grad is not None:
            grad_t=(1-np.square(t_tanh.data))*t_tanh.grad
            t.accumulate_grad(grad_t)
